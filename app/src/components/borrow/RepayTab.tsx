	...
	React.useEffect(() => {
		...
		async function scan() {
			for (let i = Number(nextLoanId) - 1; i >= 0; i--) {
				try {
					const data = await client.readContract({ address: config.polyLend, abi: polyLendAbi, functionName: 'loans', args: [BigInt(i)] }) as LoanInfo['data']
			...
			{repayConfirmed ? (
				<div className="flex flex-col gap-[12px] animate-in">
					<div className="flex flex-col items-center gap-[8px] py-[12px]">
						...
						<span className="text-[14px] font-medium text-primary">Loan repaid</span>
						<span className="text-[13px] text-tertiary">Collateral returned to your wallet</span>
					</div>
					{repayTxHash && (
